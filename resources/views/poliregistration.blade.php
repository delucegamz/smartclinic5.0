...
@section('content')
<div class="content-title"><img src="{{URL::asset('assets/images/title-poli-registration.png')}}" alt="Daftar Poli" /></div>
                    		...
                    		<div class="col-xs-10">
                    			<div class="input-group">
 			 						...
 			 						<select name="poli" id="poli" class="form-control required">
 			 							...
 			 							<option value="{{ $p->id_poli }}"{{ selected( 'Umum', $p->nama_poli ) }}>{{ $p->nama_poli }}</option>
 			 							<?php endforeach; ?>
 			 						</select>
								</div>
                                <div class="error-placement"></div>
...
$(document).ready(function(){
	            	...
	            	if(json.success == 'true'){
	            		...
	            		if($state == 'add'){
		            		...
		            		$html = '<tr class="item" id="item-' + json.id_pendaftaran + '">\
								...
								<td class="column-catatan">' + json.catatan + '</td>\
								<td class="column-action">\
									<div class="action-item first">\
							...
							$('tr#item-' + $id).find('td.column-catatan').html(json.catatan);// Catatan
	...
	$('#modal-add-item').on('hidden.bs.modal', function(e){
        @php
        $poli = App\Poli::where( 'nama_poli', '=', 'Umum')->first();
        @endphp

        $poli_id = {{ ( ( $poli && isset( $poli->id_poli ) ) ? $poli->id_poli : 0 ) }};
        ...
        $('#id_peserta').val('');
        $('#poli').val('');
        $('#poli').find('option[value="' + $poli_id + '"]').attr('selected', true);
		...
		if($id != ''){
            ...
            $.ajax({
                ...
                success: function(json) {
                    ...
                    $('#sex').find('option[value="' + json.sex.toLowerCase() + '"]').attr('selected', true);
                    $('#department').find('option[value="' + json.department + '"]').attr('selected', true);
                    $('#factory').val(json.pabrik);
                    $('#client').val(json.perusahaan);
    ...
    $('#department').change(function(){
        ...
        $client = $opt.attr('data-client');
        $factory = $opt.attr('data-factory');
