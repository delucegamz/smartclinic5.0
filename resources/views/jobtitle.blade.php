...
@section('page_title')
Smart Clinic System - Data Jabatan
@stop

@section('content')
<div class="content-title"><img src="{{URL::asset('assets/images/title-jobtitle.png')}}" alt="Daftar Poli" /></div>
...
<div class="content-top-action clearfix">
	...
	<div class="search-wrapper">
		<form method="get" action="{{ route( 'jobtitle.index' ) }}">
		...
		<table class="table table-bordered table-striped list-table" id="list-items">
			<thead>
				<tr>
					...
					<th class="column-code-title">Kode Jabatan</th>
					<th class="column-name-title">Nama Jabatan</th>
			...
			<tbody>
				...
				?>
				<tr class="item" id="item-{{ $title->id_jabatan }}">
					<td class="column-no">{{ $i }}</td>
					<td class="column-code">{{ $title->kode_jabatan }}</td>
					<td class="column-name">{{ $title->nama_jabatan }}</td>
					<td class="column-action">
						<div class="action-item first">
							<a href="#" title="Edit" class="edit" data-id="{{ $title->id_jabatan }}" data-code="{{ $title->kode_jabatan }}"><img src="{{URL::asset('assets/images/icon-file.png')}}" alt="Edit" /></a>
						</div>
						<div class="action-item last">
							<a href="#" title="Delete" class="delete" data-id="{{ $title->id_jabatan }}"><img src="{{URL::asset('assets/images/icon-delete.png')}}" alt="Delete" /></a>
						</div>
					</td>
				<tr>
				<?php
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-body">
                    ...
                    <form id="add-item" action="{{ route( 'jobtitle.store' ) }}" method="post" class="form-horizontal">
                    	<input name="_token" type="hidden" value="{{ csrf_token() }}"/>
                    	<div class="form-group">
                    		<label class="control-label col-xs-2" for="code">Kode</label>
                    		<div class="col-xs-7">
                    			<div class="input-group">
  									<span class="input-group-addon" id="code-icon"><img src="{{URL::asset('assets/images/icon-barcode.png')}}" alt="Icon Barcode" /></span>
                    	...
                    	<div class="form-group">
                    		...
                    		<div class="col-xs-10">
                    			<div class="input-group">
  									<span class="input-group-addon" id="name-icon"><img src="{{URL::asset('assets/images/icon-id.png')}}" alt="Icon ID Card" /></span>
 			 						<input type="text" class="form-control required" placeholder="Nama Jabatan" aria-describedby="name-icon" name="name" id="name" />
								</div>
                    		</div>
                    	</div>
...
$(document).ready(function(){
	            ...
	            success: function(json) {
	            	if(json.success == 'true'){
	            		...
	            		if($state == 'add'){
		            		...
		            		$html = '<tr class="item" id="item-' + json.id_jabatan + '">\
								<td class="column-no">' + $count_item + '</td>\
								<td class="column-code">' + json.kode_jabatan + '</td>\
								<td class="column-name">' + json.nama_jabatan + '</td>\
								<td class="column-action">\
									<div class="action-item first">\
										<a href="#" title="Edit" data-id="' + json.id_jabatan + '" data-code="' + json.kode_jabatan + '" class="edit"><img src="{{URL::asset('assets/images/icon-file.png')}}" alt="Edit" /></a>\
									</div>\
									<div class="action-item last">\
										<a href="#" title="Delete" data-id="' + json.id_jabatan + '" class="delete"><img src="{{URL::asset('assets/images/icon-delete.png')}}" alt="Delete" /></a>\
									</div>\
								</td>\
							<tr>';
							...
							$('tr#item-' + $id).find('td.column-name').html(json.nama_jabatan);

							$.ajax({
					            url: $('#add-item').attr('action') + '/latest_id',
					            type: 'GET',
					            data: {},
					            dataType: 'json',
					            beforeSend: function() {
	...
	$('#list-items .item .column-action a.delete').live('click', function(){
		...
		$confirm = confirm('Anda yakin ingin menghapus jabatan ini?');
