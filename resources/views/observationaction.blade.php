...
@section('page_title')
Smart Clinic System - Data Pemeriksaan Observasi
@stop

@section('content')
<div class="content-title"><img src="{{URL::asset('assets/images/title-observation-action.png')}}" alt="Daftar Pemeriksaan Observasi" /></div>
...
<div class="content-top-action clearfix">
	...
	<div class="search-wrapper">
		<form method="get" action="{{ route( 'action-observation.index' ) }}">
		...
		<table class="table table-bordered table-striped list-table" id="list-items">
			<thead>
				<tr>
					...
					<th class="column-code-title">Kode</th>
					<th class="column-name-title">Nama</th>
					<th class="column-desc-title">Uraian</th>
			...
			<tbody>
				<?php
					if( count( $datas ) ){ 
						...
						foreach ( $datas as $d ) {
				?>
				<tr class="item" id="item-{{ $d->id_pemeriksaan_observasi }}">
					<td class="column-no">{{ $i }}</td>
					<td class="column-code">{{ $d->kode_pemeriksaan_observasi }}</td>
					<td class="column-name">{{ $d->nama_pemeriksaan_observasi }}</td>
					<td class="column-name">{{ $d->nama_pemeriksaan_observasi }}</td>
					<td class="column-action">
						<div class="action-item first">
							<a href="#" title="Edit" class="edit" data-id="{{ $d->id_pemeriksaan_observasi }}" data-code="{{ $d->kode_pemeriksaan_observasi }}"><img src="{{URL::asset('assets/images/icon-file.png')}}" alt="Edit" /></a>
						</div>
						<div class="action-item last">
							<a href="#" title="Delete" class="delete" data-id="{{ $d->id_pemeriksaan_observasi }}"><img src="{{URL::asset('assets/images/icon-delete.png')}}" alt="Delete" /></a>
						</div>
					</td>
				<tr>
				<?php
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-body">
                    ...
                    <form id="add-item" action="{{ route( 'action-observation.store' ) }}" method="post" class="form-horizontal">
                    	...
                    	<div class="form-group">
                    		...
                    		<div class="col-xs-10">
                    			<div class="input-group">
 			 						...
 			 						<input type="text" class="form-control" placeholder="Nama Pemeriksaan" aria-describedby="name-icon" name="name" id="name" />
								</div>
								<div class="error-placement"></div>
                    		</div>
                    	</div>
                    	<div class="form-group">
                    		<label class="control-label col-xs-2" for="desc">Uraian</label>
                    		<div class="col-xs-10">
                    			<div class="input-group">
  									<span class="input-group-addon" id="desc-icon"><i class="fa fa-file-text"></i></span>
 			 						<textarea class="form-control" placeholder="Uraian" aria-describedby="desc-icon" name="desc" id="desc"></textarea>
								</div>
								<div class="error-placement"></div>
                    		</div>
                    	</div>
...
$(document).ready(function(){
	            ...
	            success: function(json) {
	            	if(json.success == 'true'){
	            		...
	            		if($state == 'add'){
		            		...
		            		$html = '<tr class="item" id="item-' + json.id_pemeriksaan_observasi + '">\
								<td class="column-no">' + $count_item + '</td>\
								<td class="column-code">' + json.kode_pemeriksaan_observasi + '</td>\
								<td class="column-name">' + json.nama_pemeriksaan_observasi + '</td>\
								<td class="column-desc">' + json.uraian + '</td>\
								<td class="column-action">\
									<div class="action-item first">\
										<a href="#" title="Edit" data-id="' + json.id_pemeriksaan_observasi + '" data-code="' + json.kode_pemeriksaan_observasi + '" class="edit"><img src="{{URL::asset('assets/images/icon-file.png')}}" alt="Edit" /></a>\
									</div>\
									<div class="action-item last">\
										<a href="#" title="Delete" data-id="' + json.id_pemeriksaan_observasi + '" class="delete"><img src="{{URL::asset('assets/images/icon-delete.png')}}" alt="Delete" /></a>\
									</div>\
								</td>\
							<tr>';
							...
							$('tr#item-' + $id).find('td.column-name').html(json.nama_pemeriksaan_observasi);
							$('tr#item-' + $id).find('td.column-code').html(json.kode_pemeriksaan_observasi);
							$('tr#item-' + $id).find('td.column-desc').html(json.uraian);
							$('tr#item-' + $id).find('a.edit').attr('data-code',json.kode_pemeriksaan_observasi);
	...
	$('#list-items .item .column-action a.edit').live('click', function(){
		...
		$desc = $item.find('.column-desc').html();
	...
	$('#list-items .item .column-action a.delete').live('click', function(){
		...
		$confirm = confirm('Anda yakin ingin menghapus pemeriksaan observasi ini?');
	...
	$('#rows').change(function(){
		...
		$page = $('#page').val();
		$s = $('#s').val();
		$action = '{{ route( 'action-observation.index' ) }}';
