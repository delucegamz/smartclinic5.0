...
@section('page_title')
Smart Clinic System - Data Surat Cuti
@stop
...
@section('content')
<div class="content-title"><h1>Data Surat Cuti</h1></div>
...
<div class="content-top-action clearfix full-width">
	...
	<div class="search-wrapper">
		<form method="get" action="{{ route( 'day-off-letter.index' ) }}">
			...
			<tbody>
				...
				<tr class="item" id="item-{{ $data->id_surat_cuti }}">
					...
					<td class="column-action">
						<a href="#" title="{{ ( $data->status ) ? 'Edit' : 'View' }}" class="edit" data-id="{{ $data->id_surat_cuti }}" data-code="{{ $data->id_surat_rujukan }}" data-status="{{ $data->status }}"><img src="{{URL::asset('assets/images/icon-file.png')}}" alt="Edit" /></a>
						@if( !$data->status )<a href="{{ url( 'print/dayoffletter') . '?id=' . $data->id_surat_cuti }}" title="Print" class="print" data-id="{{ $data->id_surat_cuti }}" data-code="{{ $data->id_surat_rujukan }}" data-status="{{ $data->status }}" target="_blank"><img src="{{URL::asset('assets/images/icon-print.png')}}" alt="View" /></a>@endif
					</td>
				<tr>
				<?php
							$i++;
...
$(document).ready(function(){
	...
	$('#rows').change(function(){
		...
		$page = $('#page').val();
		$s = $('#s').val();
		$action = '{{ route( 'day-off-letter.index' ) }}';
