...
@section('page_title')
Smart Clinic System - Data Golongan Obat
@stop

@section('content')
<div class="content-title"><img src="{{URL::asset('assets/images/title-medicine-group.png')}}" alt="Daftar Golongan Obat" /></div>
...
<div class="content-top-action clearfix">
	...
	<div class="search-wrapper">
		<form method="get" action="{{ route( 'medicine-group.index' ) }}">
		...
		<table class="table table-bordered table-striped list-table" id="list-items">
			<thead>
				<tr>
					...
					<th class="column-code-title">Kode Golongan Obat</th>
					<th class="column-name-title">Nama Golongan Obat</th>
			...
			<tbody>
				<?php
					if( count( $datas ) ){ 
						...
						foreach ( $datas as $medicine_group ) {
				?>
				<tr class="item" id="item-{{ $medicine_group->id_golongan_obat }}">
					<td class="column-no">{{ $i }}</td>
					<td class="column-code">{{ $medicine_group->kode_golongan_obat }}</td>
					<td class="column-name">{{ $medicine_group->nama_golongan_obat }}</td>
					<td class="column-action">
						<div class="action-item first">
							<a href="#" title="Edit" class="edit" data-id="{{ $medicine_group->id_golongan_obat }}" data-code="{{ $medicine_group->kode_golongan_obat }}"><img src="{{URL::asset('assets/images/icon-file.png')}}" alt="Edit" /></a>
						</div>
						<div class="action-item last">
							<a href="#" title="Delete" class="delete" data-id="{{ $medicine_group->id_golongan_obat }}"><img src="{{URL::asset('assets/images/icon-delete.png')}}" alt="Delete" /></a>
						</div>
					</td>
				<tr>
				<?php
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-body">
                    ...
                    <form id="add-item" action="{{ route( 'medicine-group.store' ) }}" method="post" class="form-horizontal">
                    	...
                    	<div class="form-group">
                    		...
                    		<div class="col-xs-10">
                    			<div class="input-group">
 			 						...
 			 						<input type="text" class="form-control required" placeholder="Nama Golongan Obat" aria-describedby="name-icon" name="name" id="name" />
								</div>
								<div class="error-placement"></div>
                    		</div>
                    	</div>
...
$(document).ready(function(){
	            ...
	            success: function(json) {
	            	if(json.success == 'true'){
	            		...
	            		if($state == 'add'){
		            		...
		            		$html = '<tr class="item" id="item-' + json.id_golongan_obat + '">\
								<td class="column-no">' + $count_item + '</td>\
								<td class="column-code">' + json.kode_golongan_obat + '</td>\
								<td class="column-name">' + json.nama_golongan_obat + '</td>\
								<td class="column-action">\
									<div class="action-item first">\
										<a href="#" title="Edit" data-id="' + json.id_golongan_obat + '" data-code="' + json.kode_golongan_obat + '" class="edit"><img src="{{URL::asset('assets/images/icon-file.png')}}" alt="Edit" /></a>\
									</div>\
									<div class="action-item last">\
										<a href="#" title="Delete" data-id="' + json.id_golongan_obat + '" class="delete"><img src="{{URL::asset('assets/images/icon-delete.png')}}" alt="Delete" /></a>\
									</div>\
								</td>\
							<tr>';
							...
							$('tr#item-' + $id).find('td.column-name').html(json.nama_golongan_obat);
							$('tr#item-' + $id).find('td.column-code').html(json.kode_golongan_obat);
							$('tr#item-' + $id).find('a.edit').attr('data-code',json.kode_golongan_obat);
	...
	$('#list-items .item .column-action a.delete').live('click', function(){
		...
		$confirm = confirm('Anda yakin ingin menghapus golongan obat ini?');
	...
	$('#rows').change(function(){
		...
		$page = $('#page').val();
		$s = $('#s').val();
		$action = '{{ route( 'medicine-group.index' ) }}';
