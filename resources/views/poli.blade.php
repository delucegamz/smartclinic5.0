...
@section('page_title')
Smart Clinic System - Data Poli
@stop

@section('content')
<div class="content-title"><img src="{{URL::asset('assets/images/title-poli.png')}}" alt="Daftar Poli" /></div>
		...
		<table class="table table-bordered table-striped list-table" id="list-items">
			<thead>
				<tr>
					...
					<th class="column-code-title">Kode Poli</th>
					<th class="column-name-title">Nama Poli</th>
			...
			<tbody>
				...
				?>
				<tr class="item" id="item-{{ $poli->id_poli }}">
					<td class="column-no">{{ $i }}</td>
					<td class="column-code">{{ $poli->kode_poli }}</td>
					<td class="column-name">{{ $poli->nama_poli }}</td>
					<td class="column-action">
						<div class="action-item first">
							<a href="#" title="Edit" class="edit" data-id="{{ $poli->id_poli }}" data-code="{{ $poli->kode_poli }}"><img src="{{URL::asset('assets/images/icon-file.png')}}" alt="Edit" /></a>
						</div>
						<div class="action-item last">
							<a href="#" title="Delete" class="delete" data-id="{{ $poli->id_poli }}"><img src="{{URL::asset('assets/images/icon-delete.png')}}" alt="Delete" /></a>
						</div>
					</td>
				<tr>
				<?php
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-body">
                    ...
                    <form id="add-item" action="{{ route( 'poli.store' ) }}" method="post" class="form-horizontal">
                    	...
                    	<div class="form-group">
                    		...
                    		<div class="col-xs-10">
                    			<div class="input-group">
 			 						...
 			 						<input type="text" class="form-control required" placeholder="Nama Poli" aria-describedby="name-icon" name="name" id="name" />
								</div>
								<div class="error-placement"></div>
                    		</div>
                    	</div>
	            ...
	            success: function(json) {
	            	if(json.success == 'true'){
	            		...
	            		if($state == 'add'){
		            		...
		            		$html = '<tr class="item" id="item-' + json.id_poli + '">\
								<td class="column-no">' + $count_item + '</td>\
								<td class="column-code">' + json.kode_poli + '</td>\
								<td class="column-name">' + json.nama_poli + '</td>\
								<td class="column-action">\
									<div class="action-item first">\
										<a href="#" title="Edit" data-id="' + json.id_poli + '" data-code="' + json.kode_poli + '" class="edit"><img src="{{URL::asset('assets/images/icon-file.png')}}" alt="Edit" /></a>\
									</div>\
									<div class="action-item last">\
										<a href="#" title="Delete" data-id="' + json.id_poli + '" class="delete"><img src="{{URL::asset('assets/images/icon-delete.png')}}" alt="Delete" /></a>\
									</div>\
								</td>\
							<tr>';
							...
							$('tr#item-' + $id).find('td.column-name').html(json.nama_poli);
							$('tr#item-' + $id).find('td.column-code').html(json.kode_poli);
							$('tr#item-' + $id).find('a.edit').attr('data-code',json.kode_poli);
