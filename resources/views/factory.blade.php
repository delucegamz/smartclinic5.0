...
@section('page_title')
Smart Clinic System - Data Factory
@stop

@section('content')
<div class="content-title"><img src="{{URL::asset('assets/images/title-factory.png')}}" alt="Daftar Factory" /></div>
...
<div class="content-top-action clearfix">
	...
	<div class="search-wrapper">
		<form method="get" action="{{ route( 'factory.index' ) }}">
				...
				<tr class="item" id="item-{{ $factory->id_factory }}">
					...
					<td class="column-action">
						<div class="action-item first">
							<a href="#" title="Edit" class="edit" data-id="{{ $factory->id_factory }}" data-code="{{ $factory->kode_factory }}"><img src="{{URL::asset('assets/images/icon-file.png')}}" alt="Edit" /></a>
						</div>
						<div class="action-item last">
							<a href="#" title="Delete" class="delete" data-id="{{ $factory->id_factory }}"><img src="{{URL::asset('assets/images/icon-delete.png')}}" alt="Delete" /></a>
						</div>
					</td>
				<tr>
				<?php
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-body">
                    ...
                    <form id="add-item" action="{{ route( 'factory.store' ) }}" method="post" class="form-horizontal">
                    	...
                    	<div class="form-group">
                    		...
                    		<div class="col-xs-10">
                    			<div class="input-group">
 			 						...
 			 						<input type="text" class="form-control required" placeholder="Nama Factory" aria-describedby="name-icon" name="name" id="name" />
								</div>
								<div class="error-placement"></div>
                    		</div>
                    	</div>
...
$(document).ready(function(){
	            ...
	            success: function(json) {
	            	if(json.success == 'true'){
	            		...
	            		if($state == 'add'){
		            		...
		            		$html = '<tr class="item" id="item-' + json.id_factory + '">\
								<td class="column-no">' + $count_item + '</td>\
								<td class="column-code">' + json.kode_factory + '</td>\
								<td class="column-name">' + json.nama_factory + '</td>\
								<td class="column-action">\
									<div class="action-item first">\
										<a href="#" title="Edit" data-id="' + json.id_factory + '" data-code="' + json.kode_factory + '" class="edit"><img src="{{URL::asset('assets/images/icon-file.png')}}" alt="Edit" /></a>\
									</div>\
									<div class="action-item last">\
										<a href="#" title="Delete" data-id="' + json.id_factory + '" class="delete"><img src="{{URL::asset('assets/images/icon-delete.png')}}" alt="Delete" /></a>\
									</div>\
								</td>\
							<tr>';
							...
							$('tr#item-' + $id).find('td.column-name').html(json.nama_factory);
							$('tr#item-' + $id).find('td.column-code').html(json.kode_factory);
							$('tr#item-' + $id).find('a.edit').attr('data-code',json.kode_factory);
	...
	$('#list-items .item .column-action a.delete').live('click', function(){
		...
		$confirm = confirm('Anda yakin ingin menghapus factory ini?');
